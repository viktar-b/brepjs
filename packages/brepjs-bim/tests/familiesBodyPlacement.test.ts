...
function expectBounds(actual: Bounds3D, expected: Bounds3D): void {
...
describe('retained Families Body placement and openings', () => {
  ...
  it('retains a Wall with Door and Window cuts after the candidate opening commands', () => {
    const baseline = currentKernel === 'occt-wasm' ? nativeShapeCount() : null;
    {
      ...
      expect(getKernel().volume(expected.geometry.solids[0].wrapped)).toBeCloseTo(53, 8);
      const inputs = currentKernel === 'occt-wasm' ? nativeShapeCount() : null;
      ...
      vi.spyOn(BimModel.prototype, 'replaceProductBody').mockImplementation(function (
        ...
        expect(getKernel().volume(candidate.geometry.solids[0].wrapped)).toBeCloseTo(53, 8);
      ...
      try {
        ...
        expect(getKernel().volume(wall.geometry.solids[0].wrapped)).toBeCloseTo(53, 8);
        expectBounds(getBounds(wall.geometry.solids[0]), getBounds(expected.geometry.solids[0]));
        expect(
