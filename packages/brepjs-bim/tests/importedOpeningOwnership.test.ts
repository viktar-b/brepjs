...
function observeCutResources() {
  ...
  const copies: { shape: brepjs.AnyShape<brepjs.Dimension>; release: ReturnType<typeof vi.fn> }[] =
    [];
  const borrowed: { shape: brepjs.Solid; release: ReturnType<typeof vi.fn> }[] = [];
  const cut = brepjs.cut;
  const clone = brepjs.clone;
  const getSolids = brepjs.getSolids;
  ...
  vi.spyOn(brepjs, 'getSolids').mockImplementation((...args) => {
    const solids = getSolids(...args);
    borrowed.push(...solids.map((shape) => ({ shape, release: vi.spyOn(shape, Symbol.dispose) })));
    return solids;
  });
  return { cuts, copies, borrowed };
}
...
for (const mode of ['split', 'remove-first', 'remove-all'] as const) {
  it(`preserves successful ${mode} cuts as independently owned solids in source-item groups`, async () => {
    ...
    try {
      ...
      observed.borrowed.forEach(({ shape, release }) => {
...
it.each(['copy-error', 'copy-throw', 'validation-error'] as const)(
  ...
  async (failure) => {
    ...
    try {
      ...
      observed.cuts.forEach(({ release }) => expect(release).toHaveBeenCalledTimes(1));
      observed.borrowed.forEach(({ release }) => expect(release).toHaveBeenCalledTimes(1));
      expectArena(before, releases.length);
    } finally {
      disposeImportedModel(imported);
    }
...
for (const primaryFailure of [false, true]) {
  it.each(['before', 'after'] as const)(
    ...
    async (releasePoint) => {
      ...
      // The deliberately retained compound also owns two cached borrowed subsolids.
      const outstanding = releasePoint === 'before' ? 3 : 0;
      try {
        const imported = brepjs.unwrap(
          await fromIfc(
            await bodyFixture({
