import { clone, cut, err, getSolids, ok, validSolid, type Result, type ValidSolid } from 'brepjs';
...
export function cutImportedSolids(
  ...
  const build = (): Result<readonly ValidSolid[], BimError> => {
    ...
    // getSolids() lends cached children. Never retain them after releasing their parent.
    for (const solid of getSolids(cutResult.value)) {
