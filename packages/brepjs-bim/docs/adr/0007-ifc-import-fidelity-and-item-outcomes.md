...
The first-Body-item import defect was fixed by #2278, and tessellated reconstruction was fixed by #2273. These fixes remain part of the migration baseline. Remaining [source-observed limits](../architecture-evidence.md#reconciliation-of-the-original-eight-concerns) include closed class enumeration, combined lossy meshes, and aggregate volume computed by summing items.
...
Account for every source Body item. Each yields an owned solid, a retained lossy mesh, or an explicit diagnostic. Preserve item identity and order and the all-item traversal introduced by #2278. Unsupported physical elements remain identifiable instead of disappearing from enumeration. Reconstructed composite constituents must not also appear as duplicate direct geometry.
