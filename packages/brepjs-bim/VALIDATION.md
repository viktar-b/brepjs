...
**Status: Proposed procedure under [ADR-0006](docs/adr/0006-ifc-exchange-adapter.md).**
This section specifies the migration's required qualification evidence. It does not
claim that the current CI already runs these gates. Historical results below and the
[September 10 investigation](docs/architecture-evidence.md#ifc-hierarchy-and-preservation-investigation) remain bound to their tested files.
...
| Normative rules and industry practices | Run the pinned official rule catalog described below for applicable schemas. Record the rule revision, applied rules, exclusions with reasons, and every finding. Resolve failures and record the disposition of non-normative findings.                                                                                             |
