...
The accepted engineering scope is reconstructed as a clean declarative model: one road-girder bridge plus two keyed Occurrences of one parameterized rail-arch bridge definition. The resolved and exported model contains exactly 3 Bridges, 18 BridgeParts, and 47 typed products. All 47 pass the applicable quantitative geometry, placement, volume, material, class, containment, stable-identity, validation, and reimport checks. The requested text-to-BRep sign remains blocked by the declarative IR boundary, and human visual approval remains pending; neither is counted as passed.
...
The sign uses a CSG block-font workaround, not a BimModel fallback. Its backing plate is exact, but the current declarative IR cannot consume the materialized shape returned by `loadFont()`/`sketchText()`.
...
| Text-to-BRep sign                    |             public font-outline declarative path |                block-font CSG workaround only | **Blocked / fail**     |
...
| Font-outline bridge signs                                  | `loadFont()` + `sketchText()` from public brepjs APIs inside a declarative Family | Exact plate and readable raised `BREPJS`; surface/volume gates pass, but glyph topology is approximate                           | Declared seven-segment-style project block font in CSG IR                           | Text/profile node or owned-shape bridge in CSG IR / families evaluation          | High             | Add a serializable `csg.text()` profile node with an owned font asset, or a safe content-addressed `csg.fromShape()` boundary      |
...
The accepted 47-product bridge scope is reconstructed at 100% count/relationship coverage and all quantitative gates pass. The overall delivery is not 100% complete: the mandatory text-to-BRep sign gate is blocked, and visual approval is pending. Relative to every placed nonspatial object in the original file, coverage is 47/50 = 94% because the intentionally unscoped georeference proxy and two highway-marker assemblies are absent. Bridge and BridgePart hierarchy is 100%; the broader Site shell is simplified from 6 source Sites to 3 authored bridge Sites.
...
The smallest API additions that unlock a faithful model are: recursive civil spatial routing in `familiesToBim()` and a declarative text/profile IR node backed by an owned font. An export seam for CRS plus persistent Semantic Keys would close the broader round-trip gaps. Datum-aware analytic ProductBody descriptors would then preserve more authored curve/sweep meaning in IFC instead of tessellating it.
