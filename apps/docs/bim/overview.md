...
1. **Typed specs anchor the model.** Every `add*` call validates its spec (zod schemas; the `parse*Spec` functions are exported for standalone use) and stores a typed element. Parametric physical specs build analytical solids, with IFC encoding determined by the element type. Civil spatial elements are body-less, while explicitly arbitrary bodies such as Earthworks Fill serialize as tessellation. Families-projected civil walls and railings always retain the evaluated authored items as an `EXACT` Product Body and export every item as tessellation.
