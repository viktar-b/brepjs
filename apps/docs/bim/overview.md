...
Civil-semantic Wall and Railing routes require `bodyEvaluator` or `proxyEvaluator` to copy their authored Bodies. Without it, projection returns `FAMILIES_PRODUCT_BODY_EVALUATOR_REQUIRED`. Conventional archetype walls and railings continue to use recipe authoring. Registered wall openings are applied first. Authored items remain AUTHORITATIVE, including when a recipe could produce coincident material. All Wall and Railing Bodies export their stored items as tessellation. Other typed civil routes continue to use the reference Families' semantic envelope dimensions.
